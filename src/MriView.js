  ...
  return (
    <div className="min-h-screen bg-slate-950 p-8 flex-1">
      ...
      <div className="flex items-center justify-between mb-6 w-full">
        {/* Left: Greeting */}
        <h1 className="text-3xl text-white text-left">Hello Admin User 👋🏼,</h1>
      ...
      {/* MRI View Box */}
      <div className="mx-auto bg-slate-800 p-6 rounded-2xl shadow mb-8">
        {/* Title with Back Icon */}
        <div className="flex items-center mb-4">
          <button
            onClick={() => setPage("dashboard")}
            className="mr-3 text-gray-300 hover:text-gray-100 transition"
          >
            <FontAwesomeIcon icon={faArrowLeft} className="w-5 h-5" />
          </button>
          <h1 className="text-2xl text-white font-bold text-left">
            <span className="text-gray-300">MRI View for Patient -</span> {currentPatient.name}
          </h1>
        </div>

        <h2 className="text-sm mb-2 text-light text-left text-slate-300 max-w-[50%]">
